...
object List {
  ...
  // variation1: append element of each list into a single list
  def flatMap[A, B](as: List[A], f: A => List[B]): List[B] = foldRight(
    as,
    Nil: List[B],
    (a: A, b: List[B]) => append(f(a), b)
  )

  // variation2: map function then concat them together
  def flatMap[A, B](as: List[A], f: A => List[B]): List[B] = concatViaFoldRight(map(as,f))


  // Exercise 3.21: Implement list filter via flatMap
  def filterViaFlatMap[A](as: List[A], f: A => Boolean): List[A] = flatMap(
    as,
    (a: A) => if (f(a)) List(a) else Nil
  )

  // Exercise 3.22: Implement a function that adding two list together
  def listAdd(l1: List[Int], l2: List[Int]): List[Int] = (l1, l2) match {
    case (Cons(t1, h1), Cons(t2, h2)) => Cons(t1 + t2, listAdd(h1, h2))
    case _ => Nil
  }

  // Exercise 3.23: Implement a zip function that combine two list pairwise
  // This solution is not stack safe
  def zipWith[A, B, C](l1: List[A], l2: List[B], f: (A,B) => C): List[C] = (l1, l2) match {
    case (Cons(t1, h1), Cons(t2, h2)) => Cons(f(t1, t2), zipWith(h1, h2, f))
    case _ => Nil
  }
  ...
  def zipWith[A, B, C](l1: List[A], l2: List[B], f: (A,B) => C): List[C] = {
    @tailrec
    def go(a1: List[A], a2: List[B], a3: List[C]): List[C] = (a1, a2) match {
      case (Cons(t1, h1), Cons(t2, h2)) => go(h1, h2, Cons(f(t1, t2), a3))
      case _ => Nil
    }
    reverseViaFoldLeft(go(l1, l2, Nil))
  }


  // Exercise 3.24: Implement a function to check if a subset exist within a set
