...
import exercieAnswers.chapter10Monoid.Monoid.listMonoid
...
sealed trait Validated[+E, +A] {
  ...
  def map2[EE >: E, B, C](b: Validated[EE, B])(f: (A, B) => C)(implicit m: Monoid[List[EE]]): Validated[EE, C] = (this, b) match {
    case (Valid(a), Valid(b)) => Valid(f(a, b))
    case (Invalid(e), Valid(_)) => Invalid(e)
    case (Valid(_), Invalid(e)) => Invalid(e)
    case (Invalid(e), Invalid(e1)) => Invalid(m.combine(e, e1))
  }
}
...
object Validated {
  ...
  def traverse[E, A, B](as: List[A])(f: A => Validated[E, B]): Validated[E, List[B]] = as.foldRight(Valid(Nil): Validated[E, List[B]])((a, b) => f(a).map2(b)(_ :: _)(listMonoid))
